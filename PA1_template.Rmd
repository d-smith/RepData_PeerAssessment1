...
we use the data set we imputed missing values for.
```{r echo=TRUE}
...
Now we can split out weekend days from week days
```{r echo=TRUE}
...
```{r echo=TRUE}
by_int <- group_by(subset(md2, md2$weekday=="weekday"),interval)
interval_avg = summarize(by_int, mean_steps=mean(steps, na.rm=TRUE))
plot(interval_avg,type='l',xlab='interval',ylab='average steps',main='Average Weekday Activity Pattern')
```

Weekend activity

```{r echo=TRUE}
by_int <- group_by(subset(md2, md2$weekday=="weekend"),interval)
interval_avg = summarize(by_int, mean_steps=mean(steps, na.rm=TRUE))
plot(interval_avg,type='l',xlab='interval',ylab='average steps',main='Average Weekend Activity Pattern')
```
